...
- JavaScript: `await agent(...)` (likewise `completion`) registers the handle without waiting for its result. Await registration before reading `.id`/`.handle`; `.wait()` can be called directly on an un-awaited handle.
