...
export class AgentSession {
	...
	async applyExtensionsReload(
		...
		for (const name of freshByName.keys()) {
			if (!previousNames.has(name) && this.#toolRegistry.has(name) && this.#extensionOwnedToolNames.has(name)) {
		...
		try {
			for (const name of previousNames) {
				const oldTool = this.#toolRegistry.get(name);
				const freshTool = freshByName.get(name);
				if (!oldTool) {
					if (freshTool) {
						this.#toolRegistry.set(name, freshTool);
